...
export function monthlyCharge(yearMonth: string, subscription: Subscription | null, users: User[]): number {
    ...
    const startDate = firstDayOfMonth(new Date(year, month - 1));
    const endDate = lastDayOfMonth(new Date(year, month - 1));

    let totalCharge = 0;
    const dailyRate = subscription.monthlyPriceInCents / (endDate.getDate() - startDate.getDate() + 1);

    users.forEach(user => {

        let startActiveDate: Date;

        if (user.activatedOn < startDate) {
            startActiveDate = startDate;
        } else {
            startActiveDate = user.activatedOn;
        }

        let endActiveDate: Date;

        if (!user.deactivatedOn || user.deactivatedOn > endDate) {
            endActiveDate = endDate;
        } else {
            endActiveDate = nextDay(user.deactivatedOn);
        }

        if (startActiveDate <= endActiveDate) {
            const activeDays = (endActiveDate.getDate() - startActiveDate.getDate()) + 1;
...
function runTests() {
    ...
    const fullMonthCharge = monthlyCharge("2022-04", testSubscription, testUsers);
    console.log(`Full Month Charge: ${fullMonthCharge}`);
    assert(fullMonthCharge === 2000, "Charge for full month");

    const noUsersCharge = monthlyCharge("2022-04", testSubscription, []);
    assert(noUsersCharge === 0, "Charge for no users should be 0");

    const nullSubscriptionCharge = monthlyCharge("2022-04", null, testUsers);
    assert(nullSubscriptionCharge === 0, "Charge with null subscription should be 0");
